...
- **Shiki must be mocked at test-file level in jsdom** (Wave 8.2). `SqlCodeBlock` lazy-loads `ShikiHighlighter`, whose module creates the highlighter at load time; jsdom tests should use `vi.mock('shiki')` before rendering/importing the component path that triggers the lazy module.
- **React Hooks lint forbids effect/ref synchronization patterns** (Wave 8.2). `react-hooks/set-state-in-effect` rejects copying hook state into local conversation state via `useEffect`, and `react-hooks/refs` rejects reading `ref.current` during render. Prefer updating local state inside the submit event handler and, for session switches, use the guarded render-phase reset pattern: compare a tracked `renderedSessionId` to `activeSessionId`, then reset local state only inside that inequality branch.
- **Run `npm run lint:css` during Phase 2 UI waves** (Wave 8.2). Stylelint can surface pre-existing logical-direction and vendor-property issues from earlier shell CSS while new components are being added; keep the CSS lint gate green and document any pre-existing fixes in the wave report.
- **Endpoint forwarding needs endpoint-level regression tests** (Wave 8.2). When extending request-body fields that are forwarded into a service call, add an endpoint/router test in addition to service tests. Service-only tests do not catch bugs where the API layer drops or misbinds request data before calling the service.
