...
- **Progress:** Resolved on tested branch starting from synchronized main `841905cd182140b60ee1594a07baa9bbdc7012b6`. `GET /api/v1/history` gained a trimmed ≤200-code-point search matched case-insensitively as a literal substring over question_text/generated_sql with escaped wildcards and bound parameters; cursors became opaque with a namespace bound to a SHA-256 prefix of the normalized search (cross-filter replay returns the sanitized invalid_cursor contract; legacy unfiltered cursors remain accepted), first-page totals are filtered, and audit context records only a searched flag. The frontend sends the debounced term under filter-bound query keys with stale-response isolation, keeps Load More explicit/bounded, derives selection reconciliation, scopes list/detail errors to their panels, localizes no-match/loading-more states and renders a semantic list of independent aria-pressed buttons replacing focusable divs and fake columnheaders. One locale helper locks en/ar variant normalization and precedence (?lng= → persisted device preference → navigator → html lang → en), makes manual changes authoritative by rewriting stale parameters, survives localStorage denial, centralizes document lang/dir updates, ships an accessible EN/Arabic toggle in both shells and routes all date/number rendering through active-locale formatters with LTR SQL/code isolation. Proof: 28 new backend tests within the 2,561-test unit suite, 13 integration tests on disposable PostgreSQL, 64 Schemathesis subtests, 52+36 focused frontend suites inside the 1,232-test full run, six-case mocked EN/ar/ar-EG Chromium matrix and two live real-FastAPI paginated-search cases against disposable seeded records.
- **TDD:** RED backend filtered-pagination/wildcard/cursor-binding/validation/audit/OpenAPI contract; RED frontend multi-page-search/stale-response/selection/error-placement/semantics; RED locale precedence/persistence/variants/formatting/toggle.
- **Focused gates:** history hook/component/page suites, i18n/locale/formatting suites, focused Playwright mocked+live, ESLint/typecheck/build/CSS lint, gen:api:check parity 64=64=64, Ruff and diff check.
- **Regression subset:** P1-FR-021/022, P4-FR-094/095/098/107, XP-008 — all green inside the full Vitest run.
- **Browser/live:** Required multi-page server search plus EN/ar/ar-EG reload/formatting — delivered as recorded in evidence.
- **Isolation/cleanup/evidence:** Disposable history pages, fixed clocks/browser locale; `evidence/chunk-20-history-locale.md`.
- **Stop conditions:** Locale precedence requires product choice or search requires an unplanned API contract — neither triggered: literal substring matching was already the declared client behavior, cursor compatibility was preserved safely for unfiltered pages, and precedence followed the documented requirement order.
...
- **Progress:** Resolved on tested branch starting from synchronized main `48f775834a9c2c1fa3f4c2ea95132c1ff287d119` (the squash merge of [#318](https://github.com/RkShanks/QueryCraft/pull/318), which also reconciled CHUNK-20's ledger rows). One request-scope utility combines owned controllers, parent signals, classified deadlines and exactly-once cleanup: submit/accept/reject/regenerate propagate wire-level aborts on unmount, route/session/user replacement and CHUNK-03 deletion while late settlement is suppressed without error/toast/stale state and the frozen `session_deleted` contract keeps precedence; query operations retain CHUNK-04's authoritative backend deadline with no client deadline; the four previously signal-dropping consumed queries now propagate TanStack signals under one documented bounded ordinary deadline with a localized recoverable timeout in `ClientQueryState`; audit export gained a longer documented deadline, explicit localized Cancel, zero pre-Blob resources and revoke/remove cleanup even on click failure. A location-keyed sanitized route boundary inside the authenticated shell provides retry and permission-aware safe navigation; Workspace alerts and audit toast timers are navigation-safe. One shared bounded clipboard contract covers CodeBlockActionBar and HistoryDetail (unavailable API/denial/rejection, announced states, retry, value-safe logging) and rejected lazy Shiki work falls back to readable plain-text LTR SQL with a polite localized notice. Proof spans seven RED/GREEN/gate commits, a 1,283-test full Vitest run, ESLint/typecheck/build/CSS-lint/gen-parity/harness guards (36 classified specs), 37 backend compatibility unit tests plus Ruff, a six-case mocked Chromium matrix and two live cases against a disposable FastAPI stack destroyed afterward. No stop condition triggered. See [evidence](evidence/chunk-21-browser-async.md).
...
- **Progress:** Resolved and merged through [#320](https://github.com/RkShanks/QueryCraft/pull/320) at main `e4c3ca5aac856b6c73c41c931331bf3a482a57a5`, from tested commit `63cc50d`. Applied filters govern export requests; safe server filename/media metadata, Blob lifecycle, cancellation, failed-search recovery and no-partial-file behavior are covered by the RED/GREEN commits and local gates. Backend export/search/OpenAPI groups passed; frontend Vitest (1,327), ESLint, typecheck, build, CSS lint, generated-client parity and harness guard passed; the 4/4 mocked Chromium matrix passed; and a fresh Compose stack produced two real filtered downloads with three rows each, valid server filenames, recomputed checksums, redaction/formula absence and zero unexpected browser errors. Disposable resources were removed. Authoritative CI run `32738202877` passed backend-test and frontend-test; branch deletion and synchronized main completed. See [evidence](evidence/chunk-22-audit-download.md).
...
- **Progress:** Resolved and merged at main `2114f8a8ef2f20141c0a43d87cb81b1a3ff1c758` via [#322](https://github.com/RkShanks/QueryCraft/pull/322), from tested product/CI head `cc00c867c366e4ca7cd189e161adc31215a03046`. The sign-in surface renders distinct accessible provider loading/configured/empty/failure+retry states with sanitized EN/AR retry; both sign-out surfaces expose a truthful localized Retry; a confirmed rejected-sign-in visibility defect (identity-transition remount lost the alert) was reproduced live and fixed by deriving feedback from the persisted mutation state without touching CHUNK-07 cache semantics. A fully disposable isolated HTTPS OIDC/SAML IdP runtime under `/tmp` with generated CA, dedicated PostgreSQL/Redis/backend/frontend proved real-browser redirect/callback flows, signing-key and certificate rotation without restart, retired-key/cert rejection, wrong-audience/issuer/expiry/nonce negatives, state and assertion replay, browser-side and token-exchange outage plus recovery, and strict TLS verification (untrusted-store negative rejected). All failures returned sanitized localized redirects with zero session cookies and no auth material in DOM, console, storage or evidence. Full Vitest 1,336 passed, 7-case mocked Chromium matrix, 224 SSO + 157 auth backend tests, all lint/typecheck/build/CSS/harness/parity gates and Ruff passed. Authoritative CI run `32773267333` passed backend-test and frontend-test; branch deletion and synchronized main completed. See [evidence](evidence/chunk-23-auth-idp.md).
...
- **Progress:** Partial — deterministic closure complete from synchronized main `1bfc245448e2ff4dd7b2111065ee7ce04bdff7e9` on RED `542bd34` → GREEN `81fc501`. A 44-case deterministic matrix now covers selection/normalization, factory routing, missing/malformed configuration, unsupported providers, invalid base URLs/models, timeout/cancellation, HTTP/API failures, sanitized error mapping, no-fallback, no-secret-retention and cross-adapter query-composition compatibility. The RED phase reproduced one confirmed in-gap defect: the three non-Gemini adapters leaked raw framework exceptions on malformed 200 bodies and 4xx responses; fixed by mirroring GeminiAdapter's typed sanitized mapping with constant messages (no provider body text, no signature changes, Gemini untouched). Focused gates 143 passed, full backend unit foundation 2240 passed/365 skipped, Ruff/format/diff-check clean. Live smoke: zero invocations — Anthropic lacks credentials, Ollama runtime is absent locally, OpenAI credential exists but cost/invocation approval was declined by the owner; all three classified Setup-dependent, so IS-GAP-015 is Partial, not Resolved. Closing the remainder requires exactly one approved bounded generate call per provider in an approved environment; no code dependency remains. See [evidence](evidence/chunk-24-provider-matrix.md). PR: [#324](https://github.com/RkShanks/QueryCraft/pull/324). A focused post-merge follow-up (`phase-6/wave-19.24-provider-null-body-followup`, RED `7f1d220` → GREEN `8406f13`) reproduced and fixed raw `TypeError` leakage for malformed JSON container shapes with 18 additional data-driven regression cases; deterministic total is now 62 new cases, gates 161 focused / 2258 full-unit passed, and authoritative CI run `32789833177` passed both jobs before squash merge `3fd04341f278de61ad9fef2c97b295522dde8720` via [#325](https://github.com/RkShanks/QueryCraft/pull/325). Classification remains Partial.
...
- **Progress:** Partial — deterministic closure complete from synchronized main `3f86ada49ec97cdf8eb91f32d296d5a1763f7` via three RED/GREEN triples (`45c810e`→`43186d5`, `e09c17b`→`ec4f9d0`, `d9fdbd0`→`1dcb3cb`). Thirty new disposable-harness cases prove failed/corrupt download rejection, mandatory SHA-256 verification before extraction/install, fail-closed missing-checksum behavior, interrupt-safe temp cleanup, deterministic/idempotent reruns, HTTPS-only fail-aware transport, hostile environment-value rejection before any container command or SQL, doubled-quote password literals, state-aware partial-restore recovery (drop+restore) with ONLINE-skip idempotency, abort-before-login-config on restore failure, sanitized diagnostics with secret canaries absent, DSN injection/control-character rejection, a typed sanitized connection-failure error replacing raw driver exceptions that leaked the SA-bearing DSN, pure restore-action planning, and seed engine disposal. Focused suites 40 passed, dev-up/compose regressions 6 passed, full backend unit foundation 2289 passed/365 skipped, Ruff/format/bash -n/compose-config/diff-check clean; protected dirty baseline untouched and zero disposable containers created. Upstream publishes no checksum for either fixture, so vendor-anchored verification evidence is Setup-dependent and the gap stays Partial. PR [#327](https://github.com/RkShanks/QueryCraft/pull/327) passed authoritative CI run `32905391474` and merged as `e98f2d1eed92b95c473aab793aa620ca5ddab835`; formatting follow-up [#328](https://github.com/RkShanks/QueryCraft/pull/328) passed run `32905706341` and merged as `3236ce02d9bda8924aa43db77b456a63e26b1088`. See [evidence](evidence/chunk-25-bootstrap.md).
...
- **Progress:** Resolved from synchronized main `2082f8d3ddc17ddc2c56080c051d9ca503ebc967` on RED `18ec64e` → GREEN `59b5be4` → browser proof `06b00e3`. One shared validator now enforces finite `[0,1]` and strict `flag < block` ordering before any mutation (also driving dirty tracking), with pending duplicate suppression, a dirty-aware Reset that restores authoritative values without mutating, sanitized localized EN/AR save-error toasts, and `aria-invalid`/`aria-describedby`/`role="alert"`/`role="status"` announcements. Malformed GET/PUT responses fail closed through the unchanged CHUNK-17 validators; server 422 rejections preserve valid edits and retry deterministically; CHUNK-10 backend invariants and the 64=64=64 generated contract are untouched. Proof: 15 new page tests + 7 validator tests inside the 1,358-test full Vitest run, 34 backend detection subset tests, a 12-case mocked EN/AR 1440/768/375 Chromium matrix with zero invalid mutation requests, exactly one PUT for a valid boundary save and exactly two for rejection-then-retry, and all local gates (ESLint/typecheck/build/CSS-lint/i18n/harness 39 specs/parity/diff-check) green. See [evidence](evidence/chunk-26-detection-form.md).
- **Merge:** PR [#332](https://github.com/RkShanks/QueryCraft/pull/332) passed authoritative CI run `32922572101` at tested head `9d5ad303a1e09e67c45776fee87bf32b97fb8b3f` and squash-merged as `daa9edbe3730fd9ee04b32eabf82d126f51feb6e`.
...
- **Progress:** Resolved from synchronized main `14f538aebf388a7471bdd59b00f8ff9bfa1a12ab` through final tested head `8650ca8feb5870d21ce266cca8b64fc3c06bdf46` via [#334](https://github.com/RkShanks/QueryCraft/pull/334). The harness enforces the bounded `GET /api/v1/sessions?limit=50` contract with zero unmocked 5xx responses. A 36-case mocked EN/AR 1440/768/375 Chromium matrix asserts machine-observable loading/empty/failure/recovery, root overflow and control reachability, computed direction/raw-key absence, status/alert semantics, result pagination/internal scroll, permission-denial request suppression, dialog focus containment and browser download/object-URL cleanup; the required focused subset passed 64/64. Chrome DevTools MCP independently passed the AR 375px sentinel with zero horizontal overflow, raw keys, unexpected API requests or console errors. Full frontend gates (1,358 Vitest, 422 locale/logical-CSS, lint/typecheck/build/CSS/parity/harness/diff) and CI-equivalent backend gates (2,289 unit passed, 21 OpenAPI passed, Ruff) are green. Authoritative CI run `32990393773` passed backend-test and frontend-test. The PR squash-merged as `e4b15ae96ec0f41d0903fa5ec27aebb6da058965`, and post-merge CI run `32991308202` passed both jobs. Temporary browser/build/download output was removed; the protected dirty baseline remains untouched and unstaged. See [evidence](evidence/chunk-27-responsive.json).
...
- **Progress:** Blocked — the real API returned the sanitized hostile-rejection classification, but the value-safe Playwright Chromium scan observed a synthetic canary in DOM text and stopped immediately. Chrome DevTools MCP was unavailable; no product code changed. Provider failure, audit search/export/download, identity switching and post-navigation cleanup were not continued. See [evidence](evidence/chunk-28-browser-privacy.md). CHUNK-29 remains undispatched.
...
`CHUNK-01` through `CHUNK-27` have completed their available closure. `CHUNK-24` remains Partial pending an approved provider environment, and `CHUNK-25` remains Partial pending vendor-published fixture checksums. `CHUNK-28` started but is blocked by a reproduced product defect; CHUNK-29 has not started. Decision-gated CHUNK-30/31 remain blocked until their explicit decisions are recorded.
