...
`IS-GAP-041`, `IS-GAP-027`, and `IS-GAP-028` have implementation and non-browser verification at `bbc9ef5bc6bb0b8bc2627fb36a8cbace636e6aff`. Their [CHUNK-18 evidence](evidence/chunk-18-workspace-recovery.md) records semantic zero-row and 50-row client rendering, complete unique pagination, authoritative delete reconciliation, exact regenerate restoration, permission/context-aware connection recovery, 1,140 frontend tests and 80 FastAPI ASGI compatibility tests. They remain Pending because the managed sandbox blocked required responsive Chromium and live-network proof, cleanup, and PR creation.
