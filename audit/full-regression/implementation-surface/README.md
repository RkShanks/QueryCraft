...
- `CHUNK-10` / `IS-GAP-017` + `IS-GAP-013`: **Resolved on the tested branch** at product commit `3b6f90dd938d51bef9e04ba4f5a57afce3335b68` in [#308](https://github.com/RkShanks/QueryCraft/pull/308); authoritative backend/frontend CI passed on `c0a74d891b3a28b8b35d405a85a1969a69ec561f`. See the [Markdown](evidence/chunk-10-data-invariants.md) and [JSON](evidence/chunk-10-data-invariants.json) named PostgreSQL constraints, atomic refusal/repair, downgrade/re-upgrade, concurrent singleton, fail-closed consumer, API-only recovery and cleanup evidence. Squash merge remains the `CHUNK-11` dispatch gate.
- `CHUNK-11` / `IS-GAP-012`: **Resolved on the tested branch** at product commit `00b5ad8f266d7f6156a5c3ce6ed11b59c54f5652`; authoritative backend/frontend CI passed on `f17bf48ba7a829a1f77aea374de2fe7276491c6c`. See the [Markdown](evidence/chunk-11-session-limit.md) and [JSON](evidence/chunk-11-session-limit.json) real Redis atomic session create/delete/refresh, deterministic sequence ordering, stale-index pruning, refresh no-resurrection, local/SSO concurrency, rollback, outage and cleanup evidence. Merge remains the `CHUNK-12` dispatch gate.
