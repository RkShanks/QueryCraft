...
export const HistoryList: React.FC<HistoryListProps> = ({
  ...
  return (
    <div className="history-list flex flex-col gap-4 w-full h-full select-none">
      {/* Search input — the debounced value is owned by the page and sent to the server */}
      <div className="relative">
        <Search className="absolute start-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted" aria-hidden="true" />
        <input
          type="text"
          value={search ?? ''}
      ...
      {isEmptyState ? (
        <div
          ...
          className="p-8 text-center text-text-muted select-none flex flex-col items-center justify-center flex-1 gap-2"
        >
          <Database className="w-12 h-12 opacity-35" />
          <span>{search?.trim() ? t('history.noMatch') : t('history.empty')}</span>
        </div>
      ) : (
        /* Semantic list of independent selection buttons (not a table imitation) */
        <ul role="list" className="flex flex-col gap-3 overflow-y-auto max-h-[calc(100vh-220px)] pe-1.5 scrollbar-thin m-0 p-0 list-none">
        {items.map((item) => {
          const isSelected = item.id === selectedId;
          return (
            <li key={item.id} role="listitem" className="m-0 p-0">
              <button
                type="button"
                ...
                className={`w-full text-start p-4 rounded-xl border transition-all duration-200 cursor-pointer flex flex-col gap-3 group relative overflow-hidden select-none outline-none ${
                  isSelected
      ...
      {isLoadingMore && (
        <div role="status" aria-live="polite" className="flex justify-center pt-2 text-xs text-text-muted">
