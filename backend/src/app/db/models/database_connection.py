...
class SourceDatabaseConnection(Base):
    ...
    database_type: Mapped[DatabaseType] = mapped_column("database_type", Enum(DatabaseType, native_enum=False, values_callable=lambda obj: [item.value for item in obj]), nullable=False)
    ...
    lifecycle_state: Mapped[LifecycleState] = mapped_column(
        "lifecycle_state", Enum(LifecycleState, native_enum=False, values_callable=lambda obj: [item.value for item in obj]), nullable=False, default=LifecycleState.ACTIVE, server_default=text("'active'")
    )
    health_status: Mapped[HealthStatus] = mapped_column(
        "health_status", Enum(HealthStatus, native_enum=False, values_callable=lambda obj: [item.value for item in obj]), nullable=False, default=HealthStatus.UNTESTED, server_default=text("'untested'")
    )
    ...
    schema_introspection_status: Mapped[SchemaIntrospectionStatus] = mapped_column(
        ...
        Enum(SchemaIntrospectionStatus, native_enum=False, values_callable=lambda obj: [item.value for item in obj]),
        nullable=False,
