...
Manually acquires the Redis processing lock for a session, then attempts
submit/reject/regenerate and asserts 409. After releasing the lock,
retry succeeds.
"""
...
class TestInvariantNoConcurrent:
    ...
    @pytest.mark.asyncio
    async def test_reject_concurrent_returns_409(self, authenticated_client, redis_client):
    ...
    @pytest.mark.asyncio
    async def test_regenerate_concurrent_returns_409(self, authenticated_client, redis_client):
